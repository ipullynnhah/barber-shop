...
function teste1() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (retorno == undefined) {
      ...
      retorno.id != barbearia.cortes[i].id ||
      retorno.tipo != barbearia.cortes[i].tipo ||
      retorno.valor != barbearia.cortes[i].valor
    ) {
      showAlert(
        1,
        "Sua função não está retornando o objeto com os valores esperados."
      );
      flag = true;
    }
  }
  ...
  if (typeof retorno_nao_encontrado != "string" && !flag) {
    showAlert(
      1,
      'Se o corte não for encontrado você deve retornar a string "Corte não encontrado"'
    );
    flag = true;
  } else if (
    retorno_nao_encontrado.toLowerCase() !== "corte não encontrado" &&
    retorno_nao_encontrado.toLowerCase() !== "corte nao encontrado" &&
    !flag
  ) {
    showAlert(
      1,
      'Se o corte não for encontrado você deve retornar "Corte não encontrado"'
    );
    flag = true;
  }
...
function teste2() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (retorno == undefined) {
      ...
      retorno.id != barbearia.barbas[i].id ||
  ...
  if (typeof retorno_nao_encontrado != "string" && !flag) {
    showAlert(
      2,
      'Se a barba não for encontrada você deve retornar a string "Barba não encontrada"'
    );
    flag = true;
  } else if (
    retorno_nao_encontrado.toLowerCase() !== "barba não encontrada" &&
    retorno_nao_encontrado.toLowerCase() !== "barba nao encontrada" &&
    !flag
  ) {
    showAlert(
      2,
      'Se a barba não for encontrada você deve retornar "Barba não encontrada"'
    );
    flag = true;
  }
...
function teste3() {
  ...
  if (
    (barbearia.estaAberto && retorno == "Estamos abertos") ||
    (!barbearia.estaAberto && retorno == "Estamos fechados")
  ) {
    success(id);
...
function teste5() {
  ...
  if (retorno == undefined) {
    ...
    retorno[0].tipo == "Capitão Jack" &&
    retorno[3].tipo == "Barba Baixa"
  ) {
    success(id);
  } else {
    showAlert(
      5,
      "Sua função não está retornando um array com todas as barbas."
    );
    danger(id);
  }
}
...
function teste6() {
  ...
  if (teste1 == undefined || teste2 == undefined || teste3 == undefined) {
    ...
    if (!flag) {
      showAlert(
        6,
        "Sua função não está criando o pedido com o valores ou atributos esperados."
      );
      danger(id);
    } else {
      success(id);
...
function teste7() {
  ...
  if (teste1 == undefined || teste2 == undefined) {
    ...
    if (
      teste1[0].valor != 25 ||
      teste1[0].tipo != "teste1" ||
      teste2[1].valor != 30 ||
      teste2[1].tipo != "teste2"
    ) {
      danger(id);
      showAlert(
        7,
        "Sua função não está alterando o pedido. Tente revisar seu codigo!"
      );
    } else {
      success(id);
    }
...
function showAlert(id, mensagem) {
  ...
  document.querySelector(`#section_${id}`).insertAdjacentHTML(
    "beforeend",
